...
DynamicTextField includes callbacks to change the appearance and behavior of a row.

#### Understanding Row and Cell

A `Row` is an abstraction DynamicTextField uses which holds a **value** and contains the view `Cell`. The `Cell` manages the view and subclasses `UITableViewCell`.
...
To display a list of options, DynamicTextField includes a special section called `SelectableSection`.
...
DynamicTextField includes the `ListCheckRow` which is used for example. In the custom rows of the Examples project you can also find the `ImageCheckRow`.
...
DynamicTextField 2.0.0 introduces the very requested build-in validations feature.
...
DynamicTextField allows us to specify when validation rules should be evaluated. We can do it by setting up `validationOptions` row's property, which can have the following values:
...
It is very common that you need a row that is different from those included in DynamicTextField. If this is the case you will have to create your own row but this should not be difficult. You can read [this tutorial on how to create custom rows](https://blog.xmartlabs.com/2016/09/06/Eureka-custom-row-tutorial/) to get started. You might also want to have a look at [EurekaCommunity] which includes some extra rows ready to be added to Eureka.

### Basic custom rows

To create a row with custom behaviour and appearance you'll probably want to create subclasses of `Row` and `Cell`.

Remember that `Row` is the abstraction DynamicTextField uses, while the `Cell` is the actual `UITableViewCell` in charge of the view.
...
With those 3 styles (Normal, Inline & Picker), DynamicTextField includes:
...
#### Don't want to use DynamicTextField custom operators?
